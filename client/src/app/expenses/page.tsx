...
type AggregatedData = {
  [category: string]: AggregatedDateItem;
};
...
const Expenses = () => {
  ...
  const aggregatedData: AggregatedDateItem[] = useMemo(() => {
    const fildered: AggregatedData = expenses
        ...
        <div className="flex-grow bg-white shadow rounded-lg p-4 md:p-6">
          <ResponsiveContainer width="100%" height={400}>
            <PieChart>
              ...
              >
                {aggregatedData.map(
                  (entry: AggregatedDateItem, index: number) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={
